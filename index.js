    ...
    inbounds: [
      { 
        ...
        settings: { 
          ...
          decryption: 'none', 
          fallbacks: [
            { dest: 3001 },                          // 默认回落目标（xhttp 端口）
            { path: "/vless-argo", dest: 3002 }       // ws 路由回落目标
          ] 
        }, 
...
async function extractDomains() {
  ...
  if (ARGO_AUTH && ARGO_DOMAIN) {
    ...
    try {
      ...
      // 如果 30 秒内仍未获取到域名，则执行重启逻辑
      if (!found) {
        console.log('ArgoDomain not found within 30s, re-running bot to obtain ArgoDomain');
        if (fs.existsSync(bootLogFile)) {
          try {
            fs.unlinkSync(bootLogFile);
          } catch (e) {}
        }
...
async function generateLinks(argoDomain) {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      // 删除 VMess 和 Trojan 节点，加入 VLESS-xhttp 节点，并保留 VLESS-ws 节点
      const subTxt = `
